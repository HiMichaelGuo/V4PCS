...
public:
    ...
    /// as a fraction of the size of P ([0..1]).in
...
protected:
    ...
    // mode of operation : 0->super4pcs 2->Vpcs
    int operMode;
...
protected:
    ...
    bool ComputeRigidTransformation(const std::array<Point3D, 4>& ref,
     ...
     bool ComputeRigidTransformation_V4PCS(const std::array< std::pair<Point3D, Point3D>,4>& pairs,
                                                             const Eigen::Matrix<Scalar, 3, 1>& centroid1,
                                                             Eigen::Matrix<Scalar, 3, 1> centroid2,
                                                             Scalar max_angle,
                                                             Eigen::Ref<MatrixType> transform,
    ...
    bool Perform_N_steps(int n,
                         ...
                         std::vector<Point3D>* Q,
                         const Visitor& v);
    bool Perform_N_steps_V4pcs(std::vector<Point3D>* Q,
                             std::vector< std::pair <Eigen::Isometry3d, float> > &allPose);
    bool ExtractCongruentSet(BaseGraph* baseIt);
